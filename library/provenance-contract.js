...
class ProvenanceContract {
  ...
  async depositToGateway(gatewayAddress, tokenId, options) {
    let defaultOptions = {
      gasPrice: DEFAULT_GAS_PRICE,
      wait: true,
    };
    ...
    let gasEstimate = await this.instance.methods.depositToGateway(gatewayAddress, tokenId).estimateGas({
      from: this.ownerAddress
    });
    let eventEmitter = this.instance.methods.depositToGateway(gatewayAddress, tokenId).send({
      from: this.ownerAddress,
      gas: 219362,
      gasPrice: options.gasPrice
    });
