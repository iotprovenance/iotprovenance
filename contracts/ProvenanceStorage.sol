...
contract ProvenanceStorage is ProvenanceCore {
    ...
    function _createProvenance(uint _provId, uint _tokenId, uint[] _inputProvenanceIds, string _context) internal returns (uint index) {
    ...
    function _updateProvenance(uint _provId, uint _tokenId, uint[] _inputProvenanceIds, string _context) internal returns (bool success) {
        require(isProvenance(_provId), "Non-existent key");
